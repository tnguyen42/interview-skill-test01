...
function UserTableView() {
  ...
  const handleTableChange = (pageInfo, filters, sorter) => {
    ...
    axios.get(`${SERVER_URL}users/id/${pageInfo.current}/${pageInfo.pageSize}`, {}).then((response) => {
  ...
  const deleteUser = () => {
    ...
    axios.delete(`${SERVER_URL}users/id/${userToDel}`, {}).then((response) => {
      ...
      if (response.data.response) {
        axios.get(`${SERVER_URL}users/id/${currPageNum}/${itemPerPage}`, {}).then((response) => {
          if (response.data.response) {
            setTotalUser(response.data.data.totalNumber);
            setPagination({ total: response.data.data.totalNumber });
            setDisplayData(response.data.data.users);
  ...
  return (
    <Content style={{ margin: '24px 16px 0' }}>
      ...
      <Modal title='Alert' visible={alertVisible} onOk={deleteUser} onCancel={hideModal} okText='Yes' cancelText='No'>
