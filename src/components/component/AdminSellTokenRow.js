    ...
    (async () => {
      if (props.canDo === true) {
        if (selected) {
          try {
            setStatus('Pending...');
            let gasLimit = await getEstimatedGasLimit(tokenAddress, props.network.url, address, publicKey, amount);
  ...
  return (
    <React.Fragment>
      <Col span={2} className='pl-8'>
        <input type='checkbox' className='mt-4' onChange={(e) => setSelected(e.target.checked)} />
      </Col>
      <Col span={10} className='pr-4'>
        <Input type='text' placeholder='address' className='w-full bg-gray-200 pl-0 mt-2 input-right' value={address} />
      </Col>
      <Col span={3}>
      ...
      <Col span={5} className='pr-4'>
        <Input type='number' placeholder='0.00' className='w-full bg-gray-200 pl-0 mt-2 input-right' value={amount} />
      </Col>
      <Col span={4}>
