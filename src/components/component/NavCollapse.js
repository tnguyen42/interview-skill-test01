  ...
  return (
    ...
    >
      ...
      <div className='text-center border-b-2 border-gray-200 mt-4 pb-4'>
        ...
        <Link to='/register'>
          <button className='w-full myButton myBule text-white px-12 mt-4'>{t('Register Now')}</button>
        </Link>
      </div>
      <Collapse
        efaultActiveKey={['1']}
      ...
      >
        {CollapseData.map((item, idx) => (
          <Panel
            ...
            key={idx}
            showArrow={false}
            extra={CollapseIdx == idx ? <AiOutlineArrowUp size={20} /> : <AiOutlineArrowDown size={20} />}
          >
            <div className=''>
