    ...
    <div className='relative mb-0 bg-gray-50 h-full flex flex-col'>
      ...
      <div className='w-11/12 xl:w-5/6 m-auto flex-1'>
        <div className='w-8/12 xl:w-4/6 m-auto text-center mt-12 '>
          <div className='text-2xl lg:text-4xl font-bold'>{t('P2P: Buy/Sell Your Crypto Locally')}</div>
        ...
        <div className='w-full lg:w-8/12 flex flex-col lg:flex-row text-center my-12'>
          <div className='w-full lg:w-5/12 p-2 bg-gray-100 flex rounded-lg'>
            <button
              className={`w-6/12 px-2 py-1 font-bold ${!buysellStatus ? 'bg-green-400 text-white' : 'hover:bg-green-100'} rounded-lg cursor-pointer`}
              onClick={() => setBuysellStatus(false)}
            >
              {t('Buy')}
            </button>
            <button className={`w-6/12 px-2 py-1 font-bold rounded-lg cursor-pointer text-gray-400`} disabled>
              {t('Sell')}
            </button>
          </div>
        ...
        <div className='w-full'>
          <div className='hidden lg:block'>
            <BuySellTable orderList={orderList} isLoading={isLoading} noData={noData} showCurrency={showCurrency} />
          </div>
          <div className='block lg:hidden'>
            <BuySellList orderList={orderList} isLoading={isLoading} noData={noData} showCurrency={showCurrency} />
          </div>
