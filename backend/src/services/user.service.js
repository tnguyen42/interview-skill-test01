...
class UserService {
  ...
  static async verifyEmail(email, locale) {
    ...
    if (prev) {
      ...
      if (!result) {
        return {
          response: false,
          message: 'An error was caused during the Email Verification Code generation.',
          data: null,
        };
      }
      if (result.error) {
  ...
  static async forgotPassword(rawData) {
    ...
    const body = i18n.__(
      { phrase: 'To reset your password, please click on this link: %s', locale: rawData.locale || 'En' },
      link
    );
