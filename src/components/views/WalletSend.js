...
function WalletSend(props) {
  ...
  const sendToken = (price, limit) => {
    if (!checkConfirm(props.tokensInfo[sel].address, amount, limit, price, props.network)) return false;
  ...
  const sendTransaction = (requestData) => {
    setAuthToken(localStorage.jwtToken);
    axios.post(serverUrl + 'wallets/sendtoken', requestData).then((response) => {
  ...
  return (
    <>
        ...
        <Col className='p-4' span={24}>
          <Row>
              ...
              <Row>
                <Col span={16}>
                  ...
                  <Input
                    ...
                    placeholder='0.00'
                    prefix={
                      <a className='px-2' onClick={setAmountToMax}>
                        {t('MAX')}
                      </a>
                    }
            ...
            <Col span={8}>
              ...
              <TextArea placeholder={t('Memo')} autoSize={{ minRows: 3, maxRows: 5 }} className='mt-2' />

              <Row>
                <Col span={24}>
                  {balance < amount ? <p className='text-red'> {t('Insufficient funds.')}</p> : null}

                  <button className=' mt-4 myButton  myBule text-white  text-sm w-full' onClick={showSendModal}>
                    {t('Send')}
                  </button>
                </Col>
              </Row>
      ...
      {sendModalShow ? (
        <WalletSendModal
          amount={
            presaleMode ? `${presaleData.presaleData.fromAmount} BUSD` : `${amount} ${props.tokensInfo[sel].name}`
          }
