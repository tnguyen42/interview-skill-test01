...
function P2PTableView() {
  ...
  const onChangeCurrentSold = (e) => {
    ...
    if (checkNumber(value)) setIEOData((prevData) => ({ ...prevData, raised_amount: value }));
  };
  const onChangeTotalSupply = (e) => {
    const { value } = e.target;
    if (checkNumber(value)) setIEOData((prevData) => ({ ...prevData, total_supply: value }));
  };
  ...
  const percentSold = useCallback(() => {
    ...
    if (IEOData.id) return `${((IEOData.raised_amount / IEOData.total_supply) * 100).toFixed(5)} %`;
    return '0 %';
  }, [IEOData]);
  return (
    <Content style={{ margin: '24px 16px 0' }}>
      <div className='text-lg text-right  site-layout-background full-height' style={{ padding: 24, minHeight: 360 }}>
        ...
        <Row className='mt-8'>
          <Col span={8} offset={8}>
            <button className='mx-3 myButton  myBule text-white w-full text-sm' onClick={update}>
              {t('Update')}
            </button>
          </Col>
        </Row>
