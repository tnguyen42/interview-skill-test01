        ...
        <div className={`flex  h-20  text-black ${props.className} mt-3 text-xs xl:text-sm`}>
          <div className='flex items-center   flex-grow '>
            ...
            <Dropdown overlay={menu} placement='bottomLeft'>
              <a className='hidden md:flex flex m-4 text-gray-700'>
                <IoApps size={30} />
                <AiFillCaretDown className='text-gray-400' size={30} />
              </a>
            </Dropdown>
          </div>
            ...
            <div className='flex  items-center rightBorder h-10 '>
              <Dropdown overlay={buySubmenu} placement='bottomLeft'>
                <a className='mr-4 xl:mr-8 '>
                  {t('Buy Crypto')}
                  <AiFillCaretDown className=' inline' size={20} />
                </a>
              </Dropdown>
          ...
          <div className='hidden md:block py-5'>
            <div className='flex  items-center rightBorder h-4 h-10 '>
              {userData == undefined || userData == '' ? (
                <>
                  <Link to='/login'>
                    <button className='mx-4 xl:mx-8'>{t('Log In')}</button>
                  </Link>
                  <button className='mr-4 xl:mr-8 myButton text-white myBule px-3 xl:px-6 ' onClick={goToRegister}>
                    {t('Register')}{' '}
                  </button>
                </>
              ) : (
          ...
          <div className='hidden md:block py-5'>
            <div className='flex  items-center  h-10'>
              {localStorage.getItem('publicKey') ? (
                <Link to='/wallet/0'>
                  <button className='mx-4  myButton  myBule text-white px-3 xl:px-6  '> {t('Wallet')} </button>
                </Link>
              ) : (
                <Link to='/walletMain'>
                  <button className='mx-4  myButton  myBule text-white px-3 xl:px-6 '> {t('Wallet')} </button>
          ...
          <div className='md:hidden py-5 flex '>
            {!localStorage.getItem('userInfo') ? (
              <button className='mr-4  myButton text-white myBule px-12 text-sm' onClick={goToRegister}>
                {t('Register')}
              </button>
            ) : localStorage.getItem('publicKey') ? (
              <Link to='/wallet/0'>
                <button className='mr-4  myButton  myBule text-white  px-12 text-sm '> {t('Wallet')} </button>
              </Link>
            ) : (
              <Link to='/walletMain'>
                <button className='mr-4 myButton  myBule text-white  px-12 text-sm '> {t('Wallet')} </button>
