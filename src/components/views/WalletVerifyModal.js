          ...
          props.missPhrase.includes(idx) ? (
            <div className='p-2 mt-2 myColor1 font-bold  text-lg w-1/3' key={idx}>
              <div className='border-b-2 border-gray-500 flex'>
                ...
                <input type='text' value={item} onChange={(e) => onChangePhrase(e, idx)} className='no-outline  ' />
              </div>
            </div>
          ) : (
            <div span={8} className='p-2 mt-2 text-gray-400  text-lg w-1/3' key={idx}>
              <div className='border-b-2 border-gray-500 flex'>
                <span className='flex-none'> {`${idx + 1}. `}</span> <span>{item}</span>
              </div>
            </div>
          )
        )}
      </Row>
      {showWarn ? <p className='text-red-500 text-lg mb-2'>{t('Please check mnemonic phrases.')}</p> : null}
