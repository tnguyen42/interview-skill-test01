...
function SellToken() {
  ...
  const getMGLPrice = async () => {
    getTokenPriceInUsd(network, '0xcbAe2a4625c5CB99391D8F1a0F5121B3E5A176bb').then((response) => {
      setPrice(response);
    });
  };
        ...
        <Col className='p-4' span={24}>
          ...
          <Row>
            <Col span={24} className='text-center mt-8'>
              <button className='mx-3 myButton  myBule text-white px-5 text-sm' onClick={SendAllTokens}>
                {buttonName}
              </button>
            </Col>
          </Row>
