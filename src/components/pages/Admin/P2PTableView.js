...
function P2PTableView() {
  ...
  const deleteItem = () => {
    axios.delete(`${SERVER_URL}p2p/id/${selectedIndex}`, {}).then((response) => {
    ...
    <Content style={{ margin: '24px 16px 0' }}>
      <div className='site-layout-background full-height' style={{ padding: 24, minHeight: 360 }}>
        ...
        <Table dataSource={orderList} pagination={pagination} onChange={handleTableChange} loading={isLoading}>
          <Column
            ...
            render={(text, record) => (
              <div className='flex items-center'>
                <img className='w-5 h-5 rounded-full mr-2' src={record.profile_pic} />
                {record.seller_name}
              </div>
            )}
            key='seller_name'
          ...
          <Column
            title='Amount'
            render={(text, record) => (record.currency === 'USDT' ? record.amount_usdt : record.amount_mgl)}
            key='amount'
          />
          <Column
            title='Payment Method'
            render={(text, record) => record.payment_method.replaceAll(',', '/')}
            key='method'
          />
          ...
          <Column
            title='Facebook'
            render={(text, record) => <div className='w-40'>{record.facebook_link}</div>}
            key='facebook'
          />
          <Column
            key='btn'
      ...
      <Modal title='Add Trade' visible={showAddModal} onOk={() => addItem()} onCancel={() => setShowAddModal(false)}>
        <Form name='basic' autoComplete='off' form={form} validateTrigger='onBlur'>
          <Form.Item name='profile_pic' rules={[{ required: true, message: 'Please input image.' }]}>
            <Upload
              name='avatar'
          ...
          <Form.Item name='seller_name' rules={[{ required: true, message: 'Please input seller name' }]}>
            <Input size='large' placeholder={'Seller Name'} prefix={<UserOutlined />} />
          </Form.Item>
          ...
          <Form.Item name='amount' rules={[{ required: true, message: 'Please input amount' }]}>
            <Input size='large' placeholder={'Amount'} prefix={<WalletOutlined />} />
          </Form.Item>
          ...
          <Form.Item name='price_mgl' rules={[{ required: true, message: 'Please input price' }]}>
            <Input size='large' placeholder={'Price'} prefix={<DollarCircleOutlined />} suffix='MNT' />
          </Form.Item>

          <Form.Item name='price_usdt' rules={[{ required: true, message: 'Please input price' }]}>
            <Input size='large' placeholder={'Price'} prefix={<DollarCircleOutlined />} suffix='USD' />
          </Form.Item>

          <Form.Item name='terms_conditions'>
            <Input.TextArea size='large' placeholder={'Terms and Condition'} prefix={<EditOutlined />} row={4} />
          </Form.Item>

          <Form.Item name='facebook_link'>
            <Input size='large' placeholder={''} prefix={<FacebookOutlined />} />
          </Form.Item>
