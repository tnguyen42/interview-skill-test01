    ...
    form.validateFields().then((values) => {
      ...
      axios
        ...
        .then((response) => {
          ...
          } else openNotification(t('Failed'), t(response.data.message), false, null);
        });
    });
  };
  return (
    <WalletModal title={t(props.title)} close={() => props.setModalShow(false)}>
      <div className='walletPassword'>
        <Form form={form} name='basic' initialValues={{ remember: true }} autoComplete='off' validateTrigger='onSubmit'>
          <Form.Item
          ...
          <Button size='large' className='access-button  mt-8 mb-4' onClick={reset}>
            {t('Reset')}
          </Button>
        </Form>
      </div>
